...
//CAnimationSet CAnimationSet::Animation()
//{
//	return mAnimation;
//}
	...
	for (size_t j = 0; j < mAnimation.size(); j++)
	{
		...
		else
		{
			...
			for (int k = 1; k < animation->mKeyNum; k++)
			{
				...
				{
					float r = (animation->mpKey[k].mTime - mTime) / (animation->mpKey[k].mTime - animation->mpKey[k - 1].mTime);
...
void CMesh::Render()
{
	...
	/*頂点のインデックスの場合を指定して図形を描画する*/
	for (int i = 0; i < mFaceNum; i++)
	{
		//マテリアルを適用する
		mMaterial[mpMaterialIndex[i]]->Enabled();
		glDrawElements(GL_TRIANGLES, 3, //* mFaceNum,
			GL_UNSIGNED_INT, (mpVertexIndex + i * 3)); //mpVertexIndex
