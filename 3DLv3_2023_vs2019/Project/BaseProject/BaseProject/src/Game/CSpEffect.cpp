...
// コンストラクタ
CSpEffect::CSpEffect(const CVector& pos, const CVector& dir, float speed, float dist)
	: CBillBoardImage("Effect/star.png", ETag::eSp, ETaskPauseType::eGame)
...
// カラーを設定
void CSpEffect::SetColor(const CColor& color)
{
...
// ブレンドタイプを設定
void CSpEffect::SetBlendType(EBlend type)
{
	mMaterial.SetBlendType(type);
}

// 更新
void CSpEffect::Update()
{
	// 基底クラスの更新処理
