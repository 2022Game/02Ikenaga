...
// 炎のエフェクトのアニメーションデータ
TexAnimData CFlightFlame::msAnimData = TexAnimData(8, 8, false, 64, ANIM_TIME);

// コンストラクタ
CFlightFlame::CFlightFlame(ETag tag)
...
// デストラクタ
CFlightFlame::~CFlightFlame()
{
	SAFE_DELETE(mpCollider);
}

// 各パラメータを設定
void CFlightFlame::Setup(const CVector& pos, const CVector& dir, float speed)
{
	Position(pos);
...
// 削除フラグが立っているかどうか
bool CFlightFlame::IsDeath() const
{
	return mIsDeath;
}

// カラーを設定
void CFlightFlame::SetColor(const CColor& color)
{
...
// ブレンドタイプを設定
void CFlightFlame::SetBlendType(EBlend type)
{
	mMaterial.SetBlendType(type);
}

// 衝突処理
void CFlightFlame::Collision(CCollider* self, CCollider* other, const CHitInfo& hit)
{
