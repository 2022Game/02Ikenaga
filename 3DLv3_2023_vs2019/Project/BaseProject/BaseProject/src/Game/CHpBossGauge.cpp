...
// コンストラクタ
CHpBossGauge::CHpBossGauge(bool is3dGauge)
	: mMaxValue(100)
	, mValue(100)
	, mDamageValue(100)
...
// デストラクタ
CHpBossGauge::~CHpBossGauge()
{
}

// HPゲージを削除
void CHpBossGauge::Kill()
{
	CTask::Kill();
	mpFrameImage->Kill();
...
// 表示するかどうか設定
void CHpBossGauge::SetShow(bool isShow)
{
	// ベースクラスの表示設定処理
...
// 最大値を設定
void CHpBossGauge::SetMaxHpValue(int value)
{
	mMaxValue = value;
}

// 現在値
void CHpBossGauge::SetHpValue(int value)
{
...
// 中心位置のの割合の設定
void CHpBossGauge::SetCenterRatio(const CVector2& ratio)
{
	mCenterRatio = ratio;
}

// ワールド座標を設定
void CHpBossGauge::SetWorldPos(const CVector& worldPos)
{
	// 現在のカメラを取得
